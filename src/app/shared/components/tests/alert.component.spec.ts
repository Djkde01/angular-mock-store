...
describe('AlertComponent', () => {
  ...
  it('should render message text', () => {
    ...
    fixture.whenStable().then(() => {
      ...
      expect(alertElement.nativeElement.textContent).toContain('Test alert message');
    });
  });
  ...
  it('should show dismiss button when dismissible', () => {
    ...
    expect(dismissButton.nativeElement.getAttribute('aria-label')).toBe('Cerrar');
  });
  ...
  it('should handle different icon types correctly', () => {
    ...
    iconTypes.forEach(type => {
      ...
      const iconElement = fixture.debugElement.query(By.css('div[role="alert"]'));
      expect(iconElement).toBeTruthy();

      // Check that the icon has the correct color class
      const expectedColorClass = type === 'success' ? 'text-green-800' :
