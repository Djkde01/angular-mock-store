...
export class CardComponent {
  ...
  ngAfterContentInit() {
    // These would be set programmatically or through content projection detection
    // For now, we'll assume they're used if title is provided or content is projected
  }
  ...
  getCardClasses(): string {
    ...
    const variantClasses = {
      ...
      product: 'shadow-lg hover:shadow-xl transform hover:scale-105 overflow-hidden',
      form: 'shadow-lg',
    ...
    const interactiveClasses = this.hoverable ? 'hover:shadow-lg cursor-pointer' : '';
    const clickableClasses = this.clickable ? 'cursor-pointer' : '';

    return `${baseClasses} ${variantClasses[this.variant]} ${sizeClasses[this.size]} ${interactiveClasses} ${clickableClasses} ${this.customClasses}`.trim();
  }
