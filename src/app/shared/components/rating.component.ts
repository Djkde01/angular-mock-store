...
@Component({
  ...
  template: `
    <div class="flex items-center space-x-1" [attr.aria-label]="'Calificación: ' + rating + ' de ' + maxRating + ' estrellas'">
      <!-- Stars -->
      ...
      <!-- Count text -->
      <span *ngIf="showCount && count !== undefined" [class]="getCountTextClasses()">
        ({{ count }})
      </span>
    </div>
  `
...
export class RatingComponent {
  ...
  getStarClasses(position: number): string {
    ...
    if (starType === 'full') {
      ...
      colorClasses = this.readonly ? 'text-gray-300' : 'text-gray-300 hover:text-yellow-400';
    }
